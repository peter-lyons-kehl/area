...
/// [RefIdx] (with `'static` generic lifetimes), so that those two methods then conflict if
/// attempted to be used on [RefIdx] with a `'static` lifetime.
...
pub trait ResolvableRelative {
    ...
    /// of the relative object, and (with a little inconvenience of passing in the relative) we
